...
def store_cointegration_results(df_market_prices):
    ...
    for index, base_market in enumerate(markets[:-1]):
        serie_1 = df_market_prices[base_market].values.astype(float).tolist()

        # Get Quote Pair
        for quote_market in markets[index +1:]:
            serie_2 = df_market_prices[quote_market].values.astype(float).tolist()

            # Check cointegration
            coin_flag, hedge_ratio, half_life = calculate_cointegration(serie_1, serie_2)

            # Log Pair
            if coin_flag == 1 and half_life <= MAX_HALF_lIFE and half_life > 0:
